...
var _ = Describe("getenvoy extension clean", func() {
	...
	It("should validate value of --toolchain-container-image flag", func() {
		By("running command")
		c.SetArgs([]string{"extension", "clean", "--toolchain-container-image", "?invalid value?"})
...
`))
	...
	It("should validate value of --toolchain-container-options flag", func() {
		By("running command")
		c.SetArgs([]string{"extension", "clean", "--toolchain-container-options", "imbalanced ' quotes"})
		err := cmdutil.Execute(c)
		Expect(err).To(HaveOccurred())
...
`))
	...
	Context("inside a workspace directory", func() {
		It("should succeed", func() {
			...
			By("verifying command output")
			Expect(stdout.String()).To(Equal(fmt.Sprintf("%s/docker run -u 1001:1002 --rm -t -v %s:/source -w /source --init getenvoy/extension-rust-builder:latest clean\n", dockerDir, workspaceDir)))
